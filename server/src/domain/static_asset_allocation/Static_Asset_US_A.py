...
for stock_info in my_portfolio_list:
    ...
    ########################################################
    #매수할 수량을 계산한다!
    Amt = int(stock_money / (current_price * float(KisUS.GetExrt())))
...
my_stock_list = KisUS.GetMyStockList("KRW")
...
for stock_info in my_portfolio_list:
    ...
    if stock_amt > 0:
        ...
        if stock_now_rate != stock_target_rate:
            ...
            gap_amt = gap_money / (current_price * float(KisUS.GetExrt()))
    ...
    # 잔고에 없는 경우
        ...
        ########################################################
        # 매수할 수량을 계산한다!
        buy_amt = int(buy_money / (current_price * float(KisUS.GetExrt())))
