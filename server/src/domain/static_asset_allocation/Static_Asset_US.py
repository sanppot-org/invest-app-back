...
if is_market_open == True and is_rebalance_go:
    ...
    chat_client.send_message(
        f"{portfolio_name} ({strYM}) 장이 열려서 포트폴리오 리밸런싱 가능!!"
    )
    ...
    if is_rebalance_go == True:
        chat_client.send_message(
            f"{portfolio_name} ({strYM}) 장이 닫혀서 포트폴리오 리밸런싱 불가능!!"
        )
...
if is_rebalance_go == True and is_market_open == True:
    ...
    for stock_info in my_portfolio_list:
        ...
        if rebalance_amt < 0:
            ...
            current_price *= 0.9
            pprint.pprint(
                kis_util.make_sell_limit_order(
    ...
    for stock_info in my_portfolio_list:
        ...
        if rebalance_amt > 0:
            ...
            current_price *= 1.1
            pprint.pprint(
                kis_util.make_buy_limit_order(stock_code, rebalance_amt, current_price)
            )
