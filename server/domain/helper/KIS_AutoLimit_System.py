...
try:
    ...
    IsKRMarketOpen = KisKR.is_market_open()
    IsUSMarketOpen = KisUS.is_market_open()
...
for botOrderPath in BotOrderPathList:
                ...
                if AutoLimitData["Area"] == "KR":
                    ...
                    if IsKRMarketOpen == True:
                            ...
                            for OrderInfo in OrderList:
                                if OrderInfo["OrderNum"] == AutoLimitData[
                                    ...
                                    if (
                                        ...
                                        or AutoLimitData["IsCancel"] == "Y"
                                    ):
                                        # 그런데 미체결 수량이 있다!
                                        if (
                                            OrderInfo["OrderAmt"]
                                            != OrderInfo["OrderResultAmt"]
                                        ):
                        ...
                        if Is_Except == True or Is_In == False:
                            ...
                            # 이 경우는 위에서 구한 gap_amt = abs(AutoLimitData['TargetAmt'] - stock_amt) <- 이거 기준으로 처리를 합니다.
                        ...
                        str_msg = (
                            AutoLimitData["Id"]
                            + KisKR.GetStockName(AutoLimitData["StockCode"])
                            + " 현재주식잔고: "
                            + str(stock_amt)
                            + " 주문수량:"
                            + str(AutoLimitData["OrderAmt"])
                            + " 남은주문수량:"
                            + str(gap_amt)
                            + " 주문들어갔는지:"
                            + str(OrderOk)
                        ...
                        if gap_amt != 0:
                            ...
                            if (
                                AutoLimitData["IsCancel"] == "Y"
                                or AutoLimitData["StockType"] == "DAY_END"
                                or AutoLimitData["StockType"] == "DAY_END_TRY_ETF"
                            ):
                                ...
                                try:
                                    ...
                                    # 마지막 장 끝나기 전 시간에도 수량이 남아있다면 이때는 시장가로 마무리 !
                                        ...
                                        # 주문이 유효한 상태라면 주문을 정정해준다
                                                ...
                                                if gap_amt > 0:
                                                    ...
                                                    try:
                                                        ...
                                                        print(
                                                            "Cancel OK!",
                                                            OrderData["OrderNum2"],
                                                        )  # 이부분으로 예외를 발생시킵니다 주문이 정상이라면 예외가 발생되지 않습니다.
                                            ...
                                            # 그밖의 경우는 현재가로 주문 정정!
                                                ...
                                                if gap_amt > 0:
                                                    OrderData = KisKR.CancelModifyOrder(
                                                        AutoLimitData["StockCode"],
                                    ...
                                    # DAY_END랑 비슷하지만 ETF의 NAV와 괴리율을 고려해서 하루안에 끝내거나 다음 날로 넘김 (클래스 설명 참조)
                                        ...
                                        AutoLimitData["StockType"] == "DAY_END_TRY_ETF"
                                    ):

                                        print("GAPAMT : ", gap_amt)

                                        Nav = KisKR.GetETF_Nav(
                                            AutoLimitData["StockCode"]
                                        )
                                        current_price = KisKR.GetCurrentPrice(
                                            AutoLimitData["StockCode"]
                                        )

                                        FarRate = ((current_price - Nav) / Nav) * 100.0

                                        # 최근 괴리율 절대값 평균
                                        AvgGap = KisKR.GetETFGapAvg(
                                            AutoLimitData["StockCode"]
                                        )
                                        ...
                                        if AutoLimitData["IsCancel"] == "Y":
                                            ...
                                            if gap_amt > 0:
                                                ...
                                                if FarRate <= 0 or (AvgGap * 1.5) < abs(
                                                    FarRate
                                                ):
                                                    FinalPrice = Nav
                                                # 나머지 상황에선 현재가로 주문!!!
                                        ...
                                        # 주문이 유효한 상태라면 주문을 정정해준다
                                            ...
                                            if NowHour == TargetHour:
                                                ...
                                                # else:
                                                #    OrderData = KisKR.CancelModifyOrder(AutoLimitData['StockCode'],AutoLimitData['OrderNum'],AutoLimitData['OrderNum2'],abs(gap_amt),Nav,"MODIFY","LIMIT","BUY")
                                                ...
                                                # 불리한 경우라면 NAV로 주문수정하고 체결안되면 다음날로...
                                                # else:
                                                #    OrderData = KisKR.CancelModifyOrder(AutoLimitData['StockCode'],AutoLimitData['OrderNum'],AutoLimitData['OrderNum2'],abs(gap_amt),Nav,"MODIFY","LIMIT","SELL")
                                            ...
                                            # 그밖의 경우는 주문 정정!
                                                ...
                                                if gap_amt > 0:
                                                    ...
                                                    if FarRate <= 0 or (
                                                        ...
                                                        FarRate
                                                    ):  # or (FarRate >= 1.0): #매수에 불리한 1% 이상일때는
                                                        FinalPrice = Nav
                                                ...
                                                # 팔아야 된다!
                                                    ...
                                                    if FarRate >= 0 or (
                                                        ...
                                                        FarRate
                                                    ):  # or (FarRate <= -1.0): #매도에 불리한 -1% 이하일때는 NAV가격으로 주문!
                                                        FinalPrice = Nav
                                    ...
                                    if IS_MODIFY_ORDER == True:
                                        ...
                                        AutoLimitData["OrderTime"] = (
                                            common.GetNowDateStr(AutoLimitData["Area"])
                                    ...
                                    # 마지막 장 끝나기 전 시간에도 수량이 남아있다면 이때는 시장가로 마무리 !
                                        ...
                                        # 주문이 유효한 상태라면 주문을 정정해준다
                                            ...
                                            if NowHour == TargetHour:
                                                ...
                                                CancelOK = False
                                                try:
                                                    OrderData = KisUS.CancelModifyOrder(
                                                        AutoLimitData["StockCode"],
                                                        AutoLimitData["OrderNum2"],
                                                        abs(gap_amt),
                                                        KisUS.GetCurrentPrice(
                                            ...
                                            # 그밖의 경우는 현재가로 주문 정정!
                                                ...
                                                try:
                                                    ...
                                                    if (
                                                        OrderData == "APBK1359"
                                                    ):  # {"rt_cd":"7","msg_cd":"APBK1359","msg1":"정정취소수량이 주문수량보다 많습니다"}
                                                        OrderData = (
                                                            KisUS.CancelModifyOrder(
                                                                AutoLimitData[
                                                                    "StockCode"
    ...
    for AutoLimitData in AutoOrderList:
        ...
        if int(AutoLimitData["DelDate"]) < int(
            common.GetNowDateStr(AutoLimitData["Area"], "NONE")
        ):
    ...
    for AutoLimitData in AutoOrderList:
        ...
        try:
            ...
            print(
                ...
                AutoLimitData["StockCode"],
                " IsCancel",
                AutoLimitData["IsCancel"],
                " IsDone",
                AutoLimitData["IsDone"],
            )
