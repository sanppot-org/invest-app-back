...
def test_check_is_time_to_rebalance_pass():
    ...
    interval.check_is_time_to_rebalance(now=datetime(2024, 2, 1), last_run=datetime(2024, 1, 1))
...
def test_check_is_time_to_rebalance_last_run_is_none():
    ...
    interval = Interval(time_unit=TimeUnit.MONTH, values=[1, 2, 3])
    interval.check_is_time_to_rebalance(now=datetime(2024, 2, 1), last_run=None)
